<%@page pageEncoding="UTF-8" isELIgnored="false" %>
    <script>
        $(function(){
            $("#lawsDg").edatagrid({
                url:"${pageContext.request.contextPath}/article/findAll?classift="+"d",
                fitColumns:true,
                ...
                columns:[[
                    ...
                    {title:'操作',field:'chaozuo',width:120,align:'center',
                        formatter:function(value,rows,index){
                            return "<a href='javascript:;' class='dd' onclick=\"lawsDelete('"+rows.id+"','"+rows.imgPath+"')\" data-options=\"iconCls:'icon-cancel'\">删除</a>&nbsp;&nbsp;&nbsp;" +
                                   "<a href='javascript:;' class='dd' onclick=\"lawsUpdate('"+rows.id+"','"+rows.status+"')\" data-options=\"iconCls:'icon-edit'\">修改</a>"
                        }
                    },
                ]],
                view: detailview,
                detailFormatter: function(rowIndex, rowData){
                    return '<table><tr>' +
                        '<td rowspan=2 style="border:0"><img src="${pageContext.request.contextPath}/back/admin/image/' + rowData.imgPath + '" style="height:50px;"></td>' +
                        '<td style="border:0">' +
                        '<p>标题: ' + rowData.title + '</p>' +
                        '<p>发布时间: ' + rowData.date + '</p>' +
                        '<p>状态: ' + rowData.status + '</p>' +
                        '<p>内容: ' + rowData.content + '</p>' +
                        '<p>作者: ' + rowData.gurn.name + '</p>' +
                        '</td>' +
                        '</tr></table>';
                },
                ...
                buttons:[
                    {
                        ...
                        handler:function(){
                            //提交增加form里的内容
                            $("#lawssave").form('submit',{
                                url:"${pageContext.request.contextPath}/article/add",
                                success:function(result){
                                    //刷新table表
                                    $("#lawsDg").datagrid('reload');
                                    //关闭对话框
                                    $("#savelaws").dialog('close');
        ...
        function deleteAll(){
            //返回所有被选中的行
            var rows=$("#lawsDg").datagrid('getSelections');
            if(rows.length<=0){
    ...
    <%--表--%>
    <table id="lawsDg" ></table>
    <%--增加工具栏--%>
    <div id="laws">
        <a href="javascript:;" onclick="saveLaws()" class="easyui-linkbutton" data-options="iconCls:'icon-add'">增加</a>
        <a href="javascript:;" onclick="deleteAll()" class="easyui-linkbutton" data-options="iconCls:'icon-cancel'">批量删除</a>
    </div>

    <%--增加对话框--%>
    <div id="savelaws" data-options="href:'${pageContext.request.contextPath}/back/admin/laws/save.jsp',iconCls:'icon-save',title:'轮播图增加',width:500,height:400"></div>